...
import { Plus, Search, MapPin, Globe, Building2, User, Truck, Barcode, CheckCircle, SearchX, Pencil, Trash2, RotateCcw, ListOrdered, ShoppingBag, PackagePlus } from 'lucide-react';
...
const LowStockDtl: React.FC<OrderProps> = ({ isHovered }) => {
    ...
    const totalItems = getHotboxCountLists?.lowStockItems?.length;
        ...
        <div className="min-h-screen ">
            ...
            <div className={`-mt-24 ${isHovered ? 'pl-32 pr-14' : 'pl-16 pr-14'}`}>
                <div className="max-w-8xl mx-auto">
                    <div className="bg-white rounded-2xl shadow-lg p-6">
                        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                            <div className="flex items-center gap-4  rounded-xl">
                                ...
                                <div>
                                    <h1 className="text-3xl font-semibold text-gray-900">Low Stock Sku</h1>
                                    <p className="text-gray-500 mt-2">View your low stock sku</p>
                                </div>
                            </div>
                    ...
                    <div className='mt-8'>
                        {getHotboxCountLists?.lowStockItems?.length > 0 ? (
                            <Table
                                columns={columns}
                                data={getHotboxCountLists?.lowStockItems?.map((report, index) => ({
                                    ...report,
                                    sno: index + 1,
                                }))}
                                rowsPerPage={10}
