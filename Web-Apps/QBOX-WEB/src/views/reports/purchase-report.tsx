...
import { FileText, RotateCcw, Search, ShoppingCart, Filter, ChevronDown, ChevronUp } from 'lucide-react';
...
import { getPurchaseReport } from '@state/reportSlice';
import ReportCard, { PurchaseReportCard } from '@components/purchase-report-card';
...
const PurchaseReport: React.FC<PurchaseReportProps> = ({ isHovered }) => {

    const dispatch = useDispatch<AppDispatch>();
    const { qboxEntityList, qboxEntitySno } = useSelector((state: RootState) => state.qboxEntity);
    const { dashboardQboxEntityByauthUserList } = useSelector((state: RootState) => state.dashboardSlice);
    const { restaurantList, partnerOrderDashboardDetails } = useSelector((state: RootState) => state.restaurant);
    const { deliveryPartnerList, purchaseOrderStatusList } = useSelector((state: RootState) => state.deliveryPartners);
    const { purchaseReportList } = useSelector((state: RootState) => state.reportSlice);
    ...
    const [filterKey, setFilterKey] = useState(0); // Key to force re-render of filter components

    console.log(purchaseReportList)
    ...
    useEffect(() => {
        const fetchInitialReport = () => {
            try {
                ...
                dispatch(getPurchaseReport(payload));
        ...
        fetchLocations();
        fetchInitialReport();
    }, [dispatch, roleName, authUserId, areaSno, qboxEntitySno]); // Add all relevant dependencies
    ...
    const handleSearch = async () => {
        ...
        try {
            ...
            await dispatch(getPurchaseReport(payload));
                    ...
                    <div className="bg-white rounded-2xl shadow-lg p-6">
                        <div className="flex flex-col md:flex-row justify-between items-start gap-6">
                            ...
                            <div className="flex items-center gap-4 rounded-xl">
                                ...
                                <div>
                                    <h1 className="text-3xl font-bold text-gray-800">Purchase Report</h1>
                            ...
                            <div className="flex justify-end space-x-3">
                                {(roleName && ['Super Admin', 'Aggregator Admin'].includes(roleName)) && (
                                    <DownloadButton
                                        ...
                                        disabled={!purchaseReportList?.length || isLoading}
                                        isLoading={isLoading}
                                    />
                                )}
                                <button
                        ...
                        <div className="overflow-x-auto mt-8">
                            {isLoading ? (
                                <div className='flex flex-col items-center justify-center h-64 shadow-lg rounded-xl bg-white/50 backdrop-blur-sm border border-gray-100'>
                                    <div className="animate-spin h-12 w-12 border-4 border-green-600 border-t-transparent rounded-full mb-4" />
                                    <h3 className='text-xl font-semibold text-gray-700 mb-2'>Loading Purchase Reports...</h3>
                                    <p className='text-gray-500 text-center max-w-md'>
                                        Please wait while we fetch your purchase records
                                    </p>
                                </div>
                            ) : (
