...
const DeleteArea = (modalData: any) => {
    const onDelete = async (record: any) => {
        var data = { areaSno: record?.areaSno }
        const result = await ApiService('8911', 'post', 'delete_area', data, null);
        if (result) {
            modalData.event();
        }
    }
    return (
        <div className="modal custom-modal fade" id="delete_area" role="dialog">
