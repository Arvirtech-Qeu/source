...
const PurchaseOrders = () => {
  ...
  const searchSkuInventory = async (itemIndex: any) => {
    try {
      const params = {
        purchaseOrderDtlSno: orderDetails[itemIndex]?.purchaseOrderDtlSno,
      };
  ...
  const searchPurchaseOrderDtl = async () => {
    try {
      const params = {
        purchaseOrderSno: activeItem?.purchaseOrderSno,
      };
      const result = await ApiService(
        "8912",
        "post",
  ...
  const columns = [
    ...
    {
      title: " Partner Purchase Order Id",
      dataIndex: "partnerPurchaseOrderId",
      sorter: (a: any, b: any) =>
        a.partnerPurchaseOrderId.length - b.partnerPurchaseOrderId.length,
    },
    {
      title: " Meal Time Cd",
        ...
        <div className="dropdown table-action">
          ...
          <div className="dropdown-menu dropdown-menu-right">
            <Link
              ...
              data-bs-target="#add_purchaseOrder"
            ...
            <Link
              ...
              to="#"
              data-bs-toggle="modal"
              data-bs-target="#delete_purchaseOrder"
              onClick={() => handleOpenModal(record)}
            >
  ...
  return (
    <div className="page-wrapper">
          ...
          <div className="col-md-12">
            ...
            <div className="page-header">
              <div className="row align-items-center">
                <div className="col-8">
                  <h4 className="page-title">DELHI BELLY's Food Order to Restaurants to be delivered to Q-Box </h4>
                </div>
                  ...
                  <Col span={8} className=" p-2 overflow-auto" style={{ maxHeight: '90vh' }}>
                    ...
                    <List
                      ...
                      renderItem={(item: any) => (
                        ...
                        >
                          <Card
                            className={`w-100 shadow pointer ${item.partnerPurchaseOrderId ==
                              activeItem?.partnerPurchaseOrderId
                              ? "activeItem"
                              : ""
                              }`}
                          >
                            <p className="m-0"><b>Order Id : {item.partnerPurchaseOrderId}</b></p>
                            <p className="m-0">Deliver To : {item.qboxEntity1name}</p>
                            <p className="m-0">Food From : {item.restaurant1name}</p>
                          </Card>
                        </List.Item>
                      )}
                    />
                          ...
                          {orderDetails?.map((item: any, itemIndex: any) => (
                            <Card className="m-2 shadow">
                              <p className="d-flex justify-content-between">
                                <img alt={item.title} src="/south-indian-classics.png" height={'60px'} width={'60px'} style={{ borderRadius: '10px', padding: '10px' }} />
                                <span className="h6">{item.partnerFoodCode} - {item.foodSku1name}</span>
                                <div>
                                  <span className="h6">Order Quantity</span><Badge className="px-2" style={{ fontSize: 20, width: 40, height: 22 }} count={item.orderQuantity} />

                                </div>
                              </p>
                              <p className="d-flex justify-content-between">
                                {/* <span className="h5">QBOX SKU (FOOD) ID: {item.foodSkuSno}</span> */}
                                <span>
                                  <div
                                    role="button"
                                  ...
                                  >
                                    {item.collapse ? <div className="text-danger">
                                      ▼
                                    </div> : <><div >
                                      <button type="button" className="btn btn-success bg-white text-success" >View SKU(s) ►</button>
      ...
      <PurchaseOrderModal width={800}
        modalData={purchaseOrder}
        event={searchPurchaseOrder}
      />
      <DeletePurchaseOrder
        modalData={purchaseOrder}
        event={searchPurchaseOrder}
