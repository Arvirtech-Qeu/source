...
const DeleteRestaurant = (modalData: any) => {
    const onDelete = async (record: any) => {
        var data = { restaurantSno: record?.restaurantSno }
        const result = await ApiService('8911', 'post', 'delete_restaurant', data, null);
        if (result) {
            modalData.event();
        }
    }
    return (
        <div className="modal custom-modal fade" id="delete_restaurant" role="dialog">
