...
const DeleteInfra = (modalData: any) => {
    const onDelete = async (record: any) => {
        var data = { infraSno: record?.infraSno }
        const result = await ApiService('8912', 'post', 'delete_infra', data, null);
        if (result) {
            modalData.event();
        }
    }
    return (
        <div className="modal custom-modal fade" id="delete_infra" role="dialog">
